import sys, time
import argparse
sys.path.insert(1, '/home/zeus/Autonomous-Ai-drone-scripts/modules')
...
STATE = "track"                               # takeoff land track search
# end config
...
def track():
    ...
    while True:
        ...
        if keyboard.is_pressed('q'):  # if key 'q' is pressed 
            ...
            sys.exit(0) # Closes the loop and program
        ...
        if len(detections) > 0:
            ...
            #yaw_command = 0
            if len(MA_X) > 0: #MA- Moving Average
            ...
            #draw lidar distance
            prepare_visualisation(person_center, person_to_track, image, x_delta, y_delta, fps)

            #Capturing image/picture
            ...
            cv2.imwrite('/home/zeus/FlyCam_Media/Capture.jpg', frame)
...
def prepare_visualisation(person_center, person_to_track, image, x_delta, y_delta, fps):
    ...
    #show stats
    cv2.putText(image, "fps: " + str(round(fps,2)), (50, 50), cv2.FONT_HERSHEY_SIMPLEX , 1, (0, 0, 255), 3, cv2.LINE_AA) 
